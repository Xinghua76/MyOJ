...
public class PostThumbServiceImpl extends ServiceImpl<PostThumbMapper, PostThumb>
    ...
    public int doPostThumbInner(long userId, long postId) {
        ...
        if (oldPostThumb != null) {
            ...
            if (result) {
                // 点赞数 - 1
                result = postService.update().setSql("thumb_num = thumb_num - 1").eq("id", postId).gt("thumb_num", 0)
                        .update();
                return result ? -1 : 0;
            } else {
            ...
            if (result) {
                // 点赞数 + 1
                result = postService.update().setSql("thumb_num = thumb_num + 1").eq("id", postId).update();
                return result ? 1 : 0;
            } else {
                throw new BusinessException(ErrorCode.SYSTEM_ERROR);
            }
